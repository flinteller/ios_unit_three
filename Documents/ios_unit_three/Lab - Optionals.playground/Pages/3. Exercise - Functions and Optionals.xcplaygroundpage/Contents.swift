...
func checkAge (yourAge : String){
    print(yourAge)
    if let age = Int(yourAge) {
        if age >= 18 {
            print("Welcome!")
        } else {
...
func checkPrice (food: String) -> Double?{
    if let number = stock[food]{
        if number > 0{
            return prices[food]
        }
        else {
...
let result = checkPrice(food: "Apple")
print(result!)
